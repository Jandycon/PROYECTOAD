...
public class CustomPanelAlumno extends JPanel{
	...
	public CustomPanelAlumno(String usuario, String pass) {				
	...
	private void agregarElementos() {
		atacanteImp = new AtacanteBDImp(this.usuario,pass);
		...
		cAlum.gridy = 0;
		cAlum.gridx = 0;	
		cAlum.insets = new Insets(40,10,10,10);	
		cAlum.anchor = GridBagConstraints.NORTH;
		estePanelA.add(nomusuario,cAlum);
		...
		ActionListener esteMetodo = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				for (int i = 0; i < alumnosagregados.size(); i++) {
					atacanteImp.agregarAlumno(alumnosagregados.get(i));
				}				
				for (int j = 0; j < alumnosborrados.size(); j++) {
					atacanteImp.borrarAlumno(alumnosborrados.get(j));
				}				
				btngrabar.setEnabled(false);
				limpiar();
			}
			...
			public void mouseClicked(MouseEvent e) {
	           ...
	           //Obtener el dni del alumno seleccionado
				...
				if(e.getClickCount()==2) {						
					...
					new CustomDialogAlumno(alumnoSelec, atacanteImp, modeltablaAlum, rowIdx);
				}					
			}
		};
		return esteMetodo;
	...
	private void setTableAlumFromBD() {		
		...
		arrayListAlumnos = atacanteImp.getAllAlumnos();
