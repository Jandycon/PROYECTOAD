...
public class AtacanteBDImp implements AtacanteBDDAO{
	...
	public void agregarPrestamo(Prestamo prestamo) {
		...
		try {
			if (conectar()) {
				...
				ps.setDate(3, prestamo.getFechapres());
				ps.setDate(4, prestamo.getFechadevol());
	...
	public ArrayList<Prestamo> getAllPrestamos() {
		...
		try {
			if (conectar()) {
				...
				ResultSet rs = ps.executeQuery();
				while(rs.next()) {
					Prestamo cadaPrestamo = new Prestamo(rs.getString("alumno"), rs.getString("libro"), rs.getDate("fechapres"),rs.getDate("fechadevol"), rs.getString("estado"));
	...
	public void agregarDevolucion(Devolucion devolucion) {
		...
		try {
			if (conectar()) {
				...
				ps.setDate(3, devolucion.getFechadevol());
	...
	public ArrayList<Devolucion> getAllDevoluciones() {
		...
		try {
			if (conectar()) {				
				...
				while(rs.next()) {
					Devolucion cadaDev = new Devolucion(rs.getString("libro"),rs.getString("alumno"),  rs.getDate("fechadevol"),rs.getString("estado"));
